...
def home_chat_view(request):    
    user=request.user
    chatroom_obj=PersonalChatRoom.objects.filter(members=user).order_by('-last_updated')
    room_partners={}
...
def group_chat_view(request):    
    GroupObj=GroupChatRoom.objects.all().filter(members=request.user).order_by('-last_updated')
    ...
    if request.method=='POST':
        ...
        return HttpResponseRedirect(reverse_lazy('home_groups'))    
